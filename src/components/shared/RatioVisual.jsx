          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <div style={{ display: 'flex', gap: '3px' }}>
              {[...Array(Math.min(simpA, 8))].map((_, i) => (
                <div key={i} style={{ width: compact ? '20px' : '28px', height: compact ? '20px' : '28px', background: '#ff7043', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 2px 6px rgba(0,0,0,0.2)' }} />
              ))}
            </div>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#ff9f43' }}>{simpA}</span>
          </div>

          <span style={{ fontSize: '1.2rem', fontWeight: 900, color: '#ffc107' }}>:</span>

          {/* Part B */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <div style={{ display: 'flex', gap: '3px' }}>
              {[...Array(Math.min(simpB, 8))].map((_, i) => (
                <div key={i} style={{ width: compact ? '20px' : '28px', height: compact ? '20px' : '28px', background: '#3f51b5', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 2px 6px rgba(0,0,0,0.2)' }} />
              ))}
            </div>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#7c5cbf' }}>{simpB}</span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <div style={{ display: 'flex', gap: '3px' }}>
              {[...Array(Math.min(simpC, 8))].map((_, i) => (
                <div key={i} style={{ width: compact ? '20px' : '28px', height: compact ? '20px' : '28px', background: '#4caf50', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 2px 6px rgba(0,0,0,0.2)' }} />
              ))}
            </div>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#81c784' }}>{simpC}</span>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', width: '100%', margin: '4px 0' }}>
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(63,81,181,0.15)', padding: '6px 14px', borderRadius: '12px', border: '1px solid rgba(63,81,181,0.35)' }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, color: '#7c5cbf', fontSize: '0.9rem' }}>
